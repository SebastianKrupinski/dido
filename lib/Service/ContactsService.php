...
class ContactsService {
	...
	public function __construct (string $appName, LoggerInterface $logger, CardDavBackend $DataStore, ContactsUtile $ContactsUtile) {
		$this->logger = $logger;
	 ...
	 */
	public function findCollectionEntityByUUID(string $cid, string $uuid): ?ContactObject {
    ...
    /**
     * retrieve a list of all entities for specific collection from local storage
     * 
     * @param string $cid       Collection ID
     * 
	...
	public function listEntities(string $cid): array {
        ...
        // retrieve all entities for specific collection
        $entities = $this->ContactsUtile->listEntities($cid);
        // return collections list
		return $entities;
	 ...
	 * @return ContactObject ContactObject - successfully retrieved / null - failed to retrieve
	 */
	public function fetchCollectionEntity(string $cid, string $iid): ?ContactObject {
	 ...
	 */
	public function createCollectionEntity(string $cid, ContactObject $data): ?object {
	 ...
	 * @return object Status Object - item id, item uuid, item state token / Null - failed to create
	 */
	public function updateCollectionEntity(string $cid, string $iid, ContactObject $co): ?object {
	 ...
	 * @return bool true - successfully delete / false - failed to delete
	 */
	public function deleteCollectionEntity(string $cid, string $iid): bool {
	...
	public function toContactObject(VCard $vo): ContactObject {
        ...
        if (isset($vo->TEL)) {
            foreach($vo->TEL as $entry) {
                $t = explode(',', trim($entry->parameters()['TYPE']->getValue()));
                $tm = (isset($t[0]))? strtoupper($t[0]) : null;
                $ts = (isset($t[1]))? strtoupper($t[1]) : null;
                $co->addPhone(
                    $tm,
                    $ts, 
                    trim($entry->getValue())
