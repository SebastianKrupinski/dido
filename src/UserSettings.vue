...
<template>
	<div id="ds_settings" class="section">
		<div class="ds-section-heading">
			<EwsIcon :size="32" /><h2> {{ t('integration_ds', 'Exchange EWS Connector') }}</h2>
		</div>
		<div class="ds-content">
			<h3>{{ t('integration_ds', 'Authentication') }}</h3>
			<div v-if="state.account_connected !== '1'">
				<div>
					<label>
						{{ t('integration_ds', 'Provider ') }}
					</label>
				...
				<div v-if="state.account_provider == 'MS365'">
					<div class="fields">
						<div v-if="state.system_ms365_authrization_uri === ''">
							{{ t('integration_ds', 'Microsoft Exchange 365 configuration missing. Ask your Nextcloud administrator to configure Microsoft Exchange 365 in the EWS Connector section in the administration section.') }}
						</div>
						<div v-else class="ds-connect-ms365">
							<label>
								{{ t('integration_ds', 'Press connect and enter your account information') }}
							</label>
							<NcButton @click="onConnectMS365Click">
								<template #icon>
									<CheckIcon />
								</template>
								{{ t('integration_ds', 'Connect') }}
							</NcButton>
						</div>
					</div>
				</div>
				<div v-else>
					<div class="settings-hint">
						{{ t('integration_ds', 'Enter your Exchange Server and account information then press connect.') }}
					</div>
					<div class="fields">
						<div class="line">
							<label for="ds-account-id">
								<EwsIcon />
								{{ t('integration_ds', 'Account ID') }}
							</label>
							<input id="ds-account-id"
								v-model="state.account_id"
								type="text"
								:placeholder="t('integration_ds', 'Authentication ID for your EWS Account')"
								autocomplete="off"
								autocorrect="off"
								autocapitalize="none">
						</div>
						<div class="line">
							<label for="ds-account-secret">
								<EwsIcon />
								{{ t('integration_ds', 'Account Secret') }}
							</label>
							<input id="ds-account-secret"
								v-model="state.account_secret"
								type="password"
								:placeholder="t('integration_ds', 'Authentication secret for your EWS Account')"
								autocomplete="off"
								autocorrect="off"
								autocapitalize="none">
						</div>
						<div v-if="configureManually" class="line">
							<label for="ds-server">
								<EwsIcon />
								{{ t('integration_ds', 'Account Server') }}
							</label>
							<input id="ds-server"
								v-model="state.account_server"
								type="text"
								:placeholder="t('integration_ds', 'Account Server Address')"
								autocomplete="off"
								autocorrect="off"
								autocapitalize="none">
						</div>
						<div>
							<NcCheckboxRadioSwitch :checked.sync="configureManually" type="switch">
								{{ t('integration_ds', 'Configure server manually') }}
							</NcCheckboxRadioSwitch>
						</div>
						<div>
							<NcCheckboxRadioSwitch :checked.sync="configureMail" type="switch">
								{{ t('integration_ds', 'Configure mail app on successful connection') }}
							</NcCheckboxRadioSwitch>
						</div>
						<div class="line">
							<label class="ds-connect">
								&nbsp;
							</label>
			...
			<div v-else>
				<div class="ds-connected">
					<EwsIcon />
					<label>
						{{ t('integration_ds', 'Connected to {0} at {1}', {0:state.account_id, 1:state.account_server}) }}
					</label>
					<NcButton @click="onDisconnectClick">
						<template #icon>
							<CloseIcon />
						</template>
						{{ t('integration_ds', 'Disconnect') }}
					</NcButton>
				</div>
				<div>
					{{ t('integration_ds', 'Synchronization was last started on ') }} {{ formatDate(state.account_harmonization_start) }}
					{{ t('integration_ds', 'and finished on ') }} {{ formatDate(state.account_harmonization_end) }}
				</div>
				<br>
				<div class="ds-correlations-contacts">
					<h3>{{ t('integration_ds', 'Contacts') }}</h3>
					<div class="settings-hint">
						{{ t('integration_ds', 'Select the remote contacts folder(s) you wish to synchronize by pressing the link button next to the contact folder name and selecting the local contacts address book to synchronize to.') }}
					</div>
					<div v-if="state.system_contacts == 1">
						...
						<div v-else-if="availableRemoteContactCollections.length == 0">
							{{ t('integration_ds', 'No contacts collections where found in the connected account.') }}
						</div>
						<div v-else>
							{{ t('integration_ds', 'Loading contacts collections from the connected account.') }}
						</div>
						<br>
						<div>
							<label>
								{{ t('integration_ds', 'Synchronize ') }}
							</label>
							<NcSelect v-model="state.contacts_harmonize"
								:reduce="item => item.id"
								:options="[{label: 'Never', id: '-1'}, {label: 'Manually', id: '0'}, {label: 'Automatically', id: '5'}]" />
							<label>
								{{ t('integration_ds', 'and if there is a conflict') }}
							</label>
							<NcSelect v-model="state.contacts_prevalence"
								:reduce="item => item.id"
								:options="[{label: 'Remote', id: 'R'}, {label: 'Local', id: 'L'}, {label: 'Chronology', id: 'C'}]" />
							<label>
								{{ t('integration_ds', 'prevails') }}
							</label>
						</div>
						<br>
						<div v-if="false" style="display: flex">
							<label>
								{{ t('integration_ds', 'Syncronized these local actions to the Remote system') }}
							</label>
						...
						<div v-if="false" style="display: flex">
							<label>
								{{ t('integration_ds', 'Syncronized these remote actions to the local system') }}
							</label>
					...
					<div v-else>
						{{ t('integration_ds', 'The contacts app is either disabled or not installed. Please contact your administrator to install or enable the app.') }}
					</div>
					<br>
				</div>
				<div class="ds-correlations-events">
					<h3>{{ t('integration_ds', 'Calendars') }}</h3>
					<div class="settings-hint">
						{{ t('integration_ds', 'Select the remote calendar(s) you wish to synchronize by pressing the link button next to the calendars name and selecting the local calendar to synchronize to.') }}
					</div>
					<div v-if="state.system_events == 1">
						<ul v-if="availableRemoteEventCollections.length > 0">
							<li v-for="ritem in availableRemoteEventCollections" :key="ritem.id" class="ds-collectionlist-item">
								<CalendarIcon />
								<label>
						...
						<div v-else-if="availableRemoteEventCollections.length == 0">
							{{ t('integration_ds', 'No events collections where found in the connected account.') }}
						</div>
						<div v-else>
							{{ t('integration_ds', 'Loading events collections from the connected account.') }}
						</div>
						<br>
						<div>
							<label>
				...
				<div class="ds-correlations-tasks">
					<h3>{{ t('integration_ds', 'Tasks') }}</h3>
					<div class="settings-hint">
						{{ t('integration_ds', 'Select the remote Task(s) folder you wish to synchronize by pressing the link button next to the folder name and selecting the local calendar to synchronize to.') }}
					</div>
					<div v-if="state.system_tasks == 1">
						<ul v-if="availableRemoteTaskCollections.length > 0">
							<li v-for="ritem in availableRemoteTaskCollections" :key="ritem.id" class="ds-collectionlist-item">
								<CalendarIcon />
								<label>
						...
						<div v-else-if="availableRemoteTaskCollections.length == 0">
							{{ t('integration_ds', 'No tasks collections where found in the connected account.') }}
						</div>
						<div v-else>
							{{ t('integration_ds', 'Loading tasks collections from the connected account.') }}
						</div>
						<br>
						<div>
							<label>
				...
				<div class="ds-actions">
					<NcButton @click="onSaveClick()">
						...
						{{ t('integration_ds', 'Save') }}
					</NcButton>
					<NcButton @click="onHarmonizeClick()">
						<template #icon>
							<LinkIcon />
						</template>
						{{ t('integration_ds', 'Sync') }}
					</NcButton>
					<span v-if="false">
						<NcButton @click="onTestClick('C')">
							...
							{{ t('integration_ds', 'Create Test Data') }}
						</NcButton>
						<NcButton @click="onTestClick('D')">
							<template #icon>
								<LinkIcon />
							</template>
							{{ t('integration_ds', 'Delete Test Data') }}
						</NcButton>
					</span>
				</div>
			</div>
	...
	methods: {
		...
		onConnectMS365Click() {
			const ssoWindow = window.open(
				...
				t('integration_ds', 'Sign in Nextcloud EWS Connector'),
		...
		onDisconnectClick() {
			const uri = generateUrl('/apps/integration_ds/disconnect')
			axios.get(uri)
				.then((response) => {
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to disconnect from EWS account')
						+ ': ' + error.response?.request?.responseText
					)
				})
				.then(() => {
		...
		onHarmonizeClick() {
			const uri = generateUrl('/apps/integration_ds/harmonize')
			axios.get(uri)
				.then((response) => {
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Synchronization Failed')
						+ ': ' + error.response?.request?.responseText
					)
				})
		},
		onTestClick(action) {
			const uri = generateUrl('/apps/integration_ds/test')
			const data = {
				params: {
					action,
				},
			...
			axios.get(uri, data)
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Test Failed')
						+ ': ' + error.response?.request?.responseText
					)
				})
		},
		fetchRemoteCollections() {
			const uri = generateUrl('/apps/integration_ds/fetch-remote-collections')
			axios.get(uri)
				.then((response) => {
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to load remote collections list')
						+ ': ' + error.response?.request?.responseText
					)
				})
				.then(() => {
		...
		fetchLocalCollections() {
			const uri = generateUrl('/apps/integration_ds/fetch-local-collections')
			axios.get(uri)
				.then((response) => {
					if (response.data.ContactCollections) {
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to load local collections list')
						+ ': ' + error.response?.request?.responseText
					)
				})
				.then(() => {
		...
		fetchCorrelations() {
			const uri = generateUrl('/apps/integration_ds/fetch-correlations')
			axios.get(uri)
				.then((response) => {
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to load collection correlations list')
						+ ': ' + error.response?.request?.responseText
					)
				})
				.then(() => {
		...
		depositCorrelations() {
			const uri = generateUrl('/apps/integration_ds/deposit-correlations')
			const data = {
			...
			axios.put(uri, data)
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to save correlations') + ': ' + error.response?.request?.responseText
					)
				})
				.then(() => {
				})
		...
		fetchPreferences() {
			const uri = generateUrl('/apps/integration_ds/fetch-preferences')
			axios.get(uri)
				.then((response) => {
					if (response.data) {
				...
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to retrieve preferences')
						+ ': ' + error.response.request.responseText
					)
				})
				.then(() => {
		...
		depositPreferences(values) {
			...
			const uri = generateUrl('/apps/integration_ds/deposit-preferences')
			axios.put(uri, data)
				.then((response) => {
					showSuccess(t('integration_ds', 'Saved preferences'))
				})
				.catch((error) => {
					showError(
						t('integration_ds', 'Failed to save preferences')
						+ ': ' + error.response.request.responseText
					)
				})
				.then(() => {
